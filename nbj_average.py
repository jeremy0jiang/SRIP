...
def nbj(cur):
    ...
    #resolve only when all children are leaf
    # flag = True
    # for cd in childList:
    #      if len(cd.child_nodes()) != 0:
    #          flag = False
...
def combine(nodeList):
    ...
    for i in range(1,numOfNode):
        if len(nodeList[i].child_nodes()) != 0:
            numOfPolytomy += 1
    #flagPolytomy = False
    ...
    minRow = 0
    minColumn = QMatrix[0].argmin()


    #//////
    if (len(nodeList[minColumn].child_nodes()) > 0):
        if numOfPolytomy == 1:
            # print "---------------------------"
            # print "1", nodeList[minColumn]
            # print "first one is polytomy"
            #polytomy = nodeList[minColumn]
            QMatrix[0][minColumn] = float("inf")
            minColumn = QMatrix[0].argmin()
            # print "2", nodeList[minColumn]
            # print "&&&&&&&&&&&&&&&&&&&&&&&&"
        if numOfPolytomy > 1:
            numOfPolytomy -= 1
    ...
    #minColumn = np.unravel_index(QMatrix.argmin(), QMatrix.shape)
    ...
    while numOfNode > 1:
        ...
        if (len (closestNode.child_nodes()) > 0):
            if numOfPolytomy == 1:
                # print "---------------------------"
                # print "1", nodeList[minColumn]
                # print "first one is polytomy"
                #polytomy = closestNode
                dist_tocompare[closestTaxa] = float("inf")
                closestTaxa = dist_tocompare.argmin()
                closestNode = nodeList[closestTaxa]
                # print "2", nodeList[minColumn]
                # print "&&&&&&&&&&&&&&&&&&&&&&&&"
...
# check if children of the given node are all leaves
def check_all_leaves(node):
...
def create_matrix(nodeList):
    ...
    copy_leftList = nodeList[:]
    copy_rightList = nodeList[:]
    ...
    copy_leftList[0] = tree.seed_node
    if nodeList[0] == tree.seed_node:
        copy_rightList[0] = tree.seed_node
    else:
        copy_rightList[0] = nodeList[0].leaf_nodes()[0]
    ...
    for index in range(1, len(nodeList)):
        cur = nodeList[index]
        if len(cur.child_nodes()) == 0:
            copy_leftList[index] = cur
            copy_rightList[index] = cur
        else:
            copy_leftList[index], copy_rightList[index] = cur.child_nodes()[0].leaf_nodes()[0],cur.child_nodes()[1].leaf_nodes()[0]
            #leftList[index], rightList[index] = find_two_closest(cur)
    ...
    distanceMatrix = np.empty([numOfNode, numOfNode])
    copy_smallMatrix = np.empty([numOfNode, numOfNode])
    i = j = 0
    ...
    for i in range(0, numOfNode):
        i_left = leftList[i]
        i_right = rightList[i]
        copy_i_left = copy_leftList[i]
        copy_i_right = copy_rightList[i]
        for j in range(0, numOfNode):
            j_left = leftList[j]
            j_right = rightList[j]
            copy_j_left = copy_leftList[j]
            copy_j_right = copy_rightList[j]




            distanceMatrix[i][j] = 0.5 * (
            ...
            copy_smallMatrix[i][j] = 0.5 * (
            Matrix[taxa_dictionary[copy_i_left]][taxa_dictionary[copy_j_right]] + Matrix[taxa_dictionary[copy_i_right]][
                taxa_dictionary[copy_j_left]] - Matrix[taxa_dictionary[copy_i_left]][taxa_dictionary[copy_i_right]] -
            Matrix[taxa_dictionary[copy_j_left]][taxa_dictionary[copy_j_right]])

            #newMatrix = distanceMatrix - copy_smallMatrix
